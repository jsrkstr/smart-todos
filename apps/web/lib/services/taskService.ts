...
export interface TaskRefinedNotifications {
  id: string;
  message: string;
  author: string;
}
...
export interface TaskRefinedData {
  ...
  replace_sub_tasks: boolean;
  notifications: TaskRefinedNotifications[]
}
        ...
        content: `You are a task optimization assistant.
          ...
          Provide output in JSON format only with these fields:
            ...
            task_updates (if action=update_task, nested fields a given below, send only fields that required update)
              ...
              notifications (array of objects with fields, send only fields that required update)
                id        (string)
        ...
        content: `You are a task breakdown assistant.
          ...
          Provide output in JSON format only with these fields:
            ...
            task_updates (if action=update_task, nested fields as described previously and new fields given below, send only fields that required update)
              ...
              replace_sub_tasks (must be boolean, this tells is existing subtasks are removed or not),
              sub_tasks (it must be an array of objects, each object having fields: title (string, required), description (string, optional), estimatedTimeMinutes (number, optional, default to 10 if not specified))
  ...
  static async updateRefinedTask(task: Task, refinedData: TaskRefinedData): Promise<Task> {
    ...
    const updates: UpdateTaskInput = {
      ...
      ...(refinedData.sub_tasks ? {
        children: {
          create: refinedData.sub_tasks.map(sub => ({
            title: sub.title,
          ...
          update: [],
          removeIds: refinedData.replace_sub_tasks ? existingChildrenIds : [],
        }
      }: {}),
      ...(notifications ? {
        notifications: {
          update: notifications.map(notification => ({
            id: notification.id,
            message: notification.message,
          })),
          create: [],
