...
export interface BehavioralPatterns {
  ...
  averageTasksCompletedPerDay: number;
  peakProductivityDayOfWeek: number; // 0-6 (Sunday-Saturday)

  // Task preferences
  preferredTaskDuration: number; // minutes
  takesBreaksRegularly: boolean;
  respondsBetterToUrgency: boolean; // Completes tasks faster when deadline is near

  // Communication patterns
  respondsQuicklyTo: string[]; // Types of interventions user responds to
  ignoresNotificationsAt: number[]; // Hours when notifications are typically ignored
  preferredNotificationFrequency: 'minimal' | 'moderate' | 'frequent';

  // Completion patterns
  completesTasksEarly: boolean;
  procrastinatesThenRushes: boolean;
  worksInBursts: boolean; // Long periods of activity followed by breaks

  // Time estimation accuracy
  underestimatesTime: boolean;
  overestimatesTime: boolean;
  averageEstimationError: number; // percentage

  // Focus patterns
  maxDeepWorkMinutes: number;
  optimalSessionLength: number;
  breakFrequencyNeeded: number; // minutes between breaks
  distractibilityByHour: Record<number, number>; // 0-23 -> 0-100 score

  // Energy patterns
  energyByHour: Record<number, number>; // 0-23 -> 0-100 score
  energyByDayOfWeek: Record<number, number>; // 0-6 -> 0-100 score
  energyAfterMeetings: number; // 0-100
  energyAfterExercise: number; // 0-100 (if exercise data available)

  // Task type preferences
  prefersCreativeWork: 'morning' | 'afternoon' | 'evening' | 'any';
  prefersAdminWork: 'morning' | 'afternoon' | 'evening' | 'any';
  avoidsCertainTaskTypes: string[]; // Tags of avoided tasks
  excellsAt: string[]; // Tags of tasks completed quickly/well
...
export class PatternAnalysisService {
   ...
   */
  async computePatterns(userId: string): Promise<BehavioralPatterns> {
   ...
   */
  private analyzeProductivityPatterns(tasks: any[], logs: any[]) {
    ...
    const averageTasksCompletedPerDay =
      daysWithTasks > 0 ? completedTasks.length / daysWithTasks : 0;
   ...
   */
  private analyzeTaskPreferences(tasks: any[]) {
    const completedTasks = tasks.filter((t) => t.completed && t.completedAt);
  ...
  private analyzeCommunicationPatterns(messages: any[]) {
    ...
    const respondsQuicklyTo: string[] = [];
    const messageTypeCounts: Record<string, { sent: number; responded: number }> = {};
    ...
    for (const msg of assistantMessages) {
      ...
      const nextUserMsg = userMessages.find(
        (um) => new Date(um.createdAt).getTime() > msgTime
      );
    ...
    const ignoresNotificationsAt: number[] = [];
    const notificationsByHour: Record<number, { sent: number; ignored: number }> = {};
    ...
    const preferredNotificationFrequency: 'minimal' | 'moderate' | 'frequent' =
   ...
   */
  private analyzeCompletionPatterns(tasks: any[]) {
    ...
    const tasksWithEstimates = completedTasks.filter(
      (t) => t.estimatedDuration && t.createdAt
    );
   ...
   */
  private analyzeFocusPatterns(pomodoros: any[]) {
    ...
    const breakFrequencyNeeded =
      breaks.length > 0
        ? Math.round(breaks.reduce((a, b) => a + b, 0) / breaks.length)
        : 30;

    // Calculate distractibility by hour (based on interrupted pomodoros)
    const distractibilityByHour: Record<number, number> = {};
    const pomodorosByHour: Record<number, { total: number; interrupted: number }> = {};
    ...
    Object.entries(pomodorosByHour).forEach(([hour, counts]) => {
      if (counts.total > 0) {
        distractibilityByHour[parseInt(hour)] = Math.round(
          (counts.interrupted / counts.total) * 100
        );
      }
    });
  ...
  private analyzeEnergyPatterns(tasks: any[], logs: any[]) {
    ...
    for (let i = 0; i < 7; i++) tasksByDay[i] = 0;
    ...
    Object.keys(tasksByHour).forEach((hour) => {
      energyByHour[parseInt(hour)] = Math.round(
        (tasksByHour[parseInt(hour)] / maxTasksPerHour) * 100
      );
    });

    Object.keys(tasksByDay).forEach((day) => {
      energyByDayOfWeek[parseInt(day)] = Math.round(
        (tasksByDay[parseInt(day)] / maxTasksPerDay) * 100
      );
    });
   ...
   */
  private analyzeTaskTypePreferences(tasks: any[]) {
    const completedTasks = tasks.filter((t) => t.completed && t.completedAt);

    if (completedTasks.length < 10) {
      return {
        prefersCreativeWork: 'any' as const,
        prefersAdminWork: 'any' as const,
    ...
    completedTasks.forEach((task) => {
      ...
      const isCreative = tags.some((t: string) =>
        ['design', 'creative', 'writing', 'brainstorm'].includes(t.toLowerCase())
      );
      const isAdmin = tags.some((t: string) =>
        ['admin', 'email', 'paperwork', 'organize'].includes(t.toLowerCase())
      );

      if (isCreative) creativeTasksByPeriod[period]++;
      if (isAdmin) adminTasksByPeriod[period]++;
    });

    const prefersCreativeWork = Object.entries(creativeTasksByPeriod).sort(
      ([, a], [, b]) => b - a
    )[0]?.[0] as 'morning' | 'afternoon' | 'evening' || 'any';

    const prefersAdminWork = Object.entries(adminTasksByPeriod).sort(
      ([, a], [, b]) => b - a
    )[0]?.[0] as 'morning' | 'afternoon' | 'evening' || 'any';

    // Find avoided task types (incomplete tasks with same tags)
    const incompleteTasks = tasks.filter((t) => !t.completed);
    const tagCounts: Record<string, { total: number; incomplete: number }> = {};
   ...
   * Store patterns in database
   */
  private async storePatterns(userId: string, patterns: BehavioralPatterns) {
   ...
   */
  async loadPatterns(userId: string): Promise<BehavioralPatterns | null> {
    ...
    return record.patterns as unknown as BehavioralPatterns;
  }
}
 ...
 */
export function createPatternAnalysisService(prisma: PrismaClient): PatternAnalysisService {
