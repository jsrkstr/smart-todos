 ...
 * ```
 */
export async function getPomodoros(filters?: {
  taskId?: string
  startDate?: string
  endDate?: string
}): Promise<PomodoroSession[]> {
  const token = getToken()
  return await callInternalTool<PomodoroSession[]>(
    'getPomodoros',
    filters || {},
    token
  )
...
export async function createPomodoro(params: {
  ...
  const token = getToken()
  return await callInternalTool<PomodoroSession>('createPomodoro', params, token)
}
