...
export class DockerSandbox {
  private containers: Map<string, SandboxEnvironment> = new Map()
  ...
  async createContainer(
    ...
    try {
      ...
      throw new Error(
        `Failed to create sandbox container: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
   ...
   */
  async cleanupUserContainers(userId: string): Promise<void> {
    const userContainers = Array.from(this.containers.values())
      .filter(env => env.userId === userId)

    await Promise.all(
      userContainers.map(env => this.cleanupContainer(env.containerId))
    )
  }
  ...
  async cleanupAll(): Promise<void> {
    ...
    await Promise.all(
      containerIds.map(id => this.cleanupContainer(id))
    )
  }
