...
export function validateCode(code, language) {
    ...
    const importMatches = code.match(/(?:import\s+.*?\s+from\s+['"]([^'"]+)['"]|require\s*\(\s*['"]([^'"]+)['"]\s*\))/g);
    if (importMatches) {
        for (const match of importMatches) {
            ...
            if (moduleMatch) {
                ...
                const isAllowed = ALLOWED_IMPORTS.some(allowed => moduleName.startsWith(allowed));
 ...
 */
export function validateCodeSize(code) {
